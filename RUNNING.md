...
| 换片后 mpv 完全不动（IPC 查 `idle-active=True`）、日志有 `param error , http_range` | 中继把「起点已在文件尾」当成前跳，发了非法的 `bytes=<size>-`。已在 `get()` / `_bypass_once()` / 泵循环里按 EOF 处理（回空数据而不是报错）。mpv 对**首次**打开流失败不重试，所以这类错误必须在中继里消化掉 |
| 日志里 `旁路失败 … 重试(1/3)` | CDN 节点偶发 4xx/掐线，中继会自己重试 3 次（索引在文件尾的 mp4 全靠这一次尾部读起播） |
